...
export function HeroSlider() {
  ...
  return (
    ...
    >
      ...
      {/* ── Slide content (each fills the full area, flex-centered) ── */}
      {SLIDES.map((slide, i) => (
        <div
          key={i}
          className={cn(
      ...
      <button
        ...
        className="hidden sm:flex absolute left-4 top-1/2 -translate-y-1/2 z-30 items-center justify-center w-10 h-10 rounded-full bg-black/30 hover:bg-black/60 text-white backdrop-blur-sm transition-colors"
      >
      ...
      <button
        ...
        className="hidden sm:flex absolute right-4 top-1/2 -translate-y-1/2 z-30 items-center justify-center w-10 h-10 rounded-full bg-black/30 hover:bg-black/60 text-white backdrop-blur-sm transition-colors"
      >
        <ChevronRight className="h-5 w-5" />
      </button>

      {/* ── Pagination dots with progress in active ── */}
        ...
        {SLIDES.map((_, i) => (
          <button
            ...
            className={cn(
              ...
              i === active
                ? "w-10 h-2 bg-white/30"
                : "w-2 h-2 bg-white/40 hover:bg-white/70"
            )}
          >
            {/* Progress fill — only on active dot; fully filled when paused */}
            {i === active && (
              <div
